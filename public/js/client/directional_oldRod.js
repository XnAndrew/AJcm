    ...
    methods : {
        ...
        fetchData : function()
        {
            var self = this;
             ...
             // window.alert(this.rpp);
            // console.log('Here is my Data: ' + (5 + 6));
            ...
            this.$http({url: '/cm/client/data/directional', method: 'GET'}).then(function (response)
            {
                ...
                if(response.data.length > 0)
                {
                    ...
                    // var newrpp = $('#eventsContainer') / rowHeight;
                    var newrpp = 3;
                    // this.totalPages = Math.ceil(response.data.length / newrpp);


                    /*
                        |Automate the number of rows per page for directional display
                        |
                        |this.totalPages = Math.ceil(response.data.length / 4);
                        |
                        |Event height / list item height = rpp
                        |eventsContainer / listitem = rpp
                        |var newrpp = $('#eventsContainer') / rowHeight;
...
// alert("Hello! I am an alert box!! " +  $('#eventsContainer').height());
// alert((729*(80/100)/121));
//var a = $('#eventsContainer').height();
// var b = this.rowHeight;
// alert(rowHeight);
// alert(Math.floor(284/121));

// alert((($('#eventsContainer').height())*(80/100)/this.rowHeight));
